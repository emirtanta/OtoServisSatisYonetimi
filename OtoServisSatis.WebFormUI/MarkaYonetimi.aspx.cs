    ...
    public partial class MarkaYonetimi : System.Web.UI.Page
    {
        ...
        protected void btnGuncelle_Click(object sender, EventArgs e)
        {
            try
            {
                if (lblId.Text != "0")
                {
                    int islemSonucu = markaManager.Update(
                    new Marka
                    {
        ...
        protected void btnSil_Click(object sender, EventArgs e)
        {
            try
            {
                if (lblId.Text != "0")
                {
                    var marka = markaManager.Get(int.Parse(lblId.Text));
                    int islemSonucu = markaManager.Delete(marka);
                    if (islemSonucu > 0)
                    {
                        Response.Redirect("MarkaYonetimi.aspx");
                    }
        ...
        protected void dgvMarkalar_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                //1=> 1 değeri id değerinin kolon indisidir
                int id = Convert.ToInt32(row.Cells[1].Text);
                if (id > 0)
                {
                    var marka = markaManager.Get(id);
